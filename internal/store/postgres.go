...
func (s *PostgresStore) CreateUser(ctx context.Context, user api.UserRecord) error {
	_, err := s.pool.Exec(ctx, `INSERT INTO artisan_users (id, username, display_name, password_hash, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $5)`, user.ID, user.Username, user.DisplayName, user.PasswordHash, user.CreatedAt)
	return err
}

func (s *PostgresStore) FindUserByUsername(ctx context.Context, username string) (api.UserRecord, error) {
	return s.readUser(ctx, `SELECT id::text, username, display_name, password_hash, created_at FROM artisan_users WHERE username=$1`, username)
}

func (s *PostgresStore) FindUserByID(ctx context.Context, id string) (api.UserRecord, error) {
	return s.readUser(ctx, `SELECT id::text, username, display_name, password_hash, created_at FROM artisan_users WHERE id=$1`, id)
}

func (s *PostgresStore) readUser(ctx context.Context, sql string, args ...any) (api.UserRecord, error) {
	var user api.UserRecord
	err := s.pool.QueryRow(ctx, sql, args...).Scan(&user.ID, &user.Username, &user.DisplayName, &user.PasswordHash, &user.CreatedAt)
	return user, err
}
